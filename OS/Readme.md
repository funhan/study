...
다음 네 가지가 동시에 만족되었을 때 데드락이 발생할 수 있다.
...
이때, `적중율(Hit rate)`을 극대화 시키기 위해 데이터 `지역성(Locality)의 원리`를 사용한다. 지역성의 전제조건으로 프로그램은 모든 코드나 데이터를 균등하게 Access하지 않는다는 특성을 기본으로 한다. 즉, `Locality`란 기억 장치 내의 정보를 균일하게 Access하는 것이 아닌 어느 한 순간에 특정 부분을 집중적으로 참조하는 특성이다.
...
언급했듯이 캐시는 프로세서 가까이에 위치하면서 빈번하게 사용되는 데이터를 저장하는 장소이다. 하지만 캐시가 아무리 가까이 있더라도 찾고자 하는 데이터가 어느 곳에 저장되어 있는지 몰라 모든 데이터를 순회해야 한다면 시간이 오래 걸리게 된다. 즉, 캐시에 목적 데이터가 저장되어 있다면 바로 접근하여 출력할 수 있어야 캐시가 의미 있어진다는 것이다.

그렇기 때문에 캐시에 데이터를 저장할 때 특정 자료 구조를 사용하여 묶음으로 저장하게 되는데 이를 `캐싱 라인`이라고 한다. 다양한 주소에 있는 데이터를 사용하므로 빈번하게 사용하는 데이터의 주소는 흩어져 있다. 따라서 캐시에 저장하는 데이터에는 데이터의 메모리 주소 등을 기록해 둔 태그를 달아놓을 필요가 있다. 이러한 태그들의 묶음을 캐싱 라인이라고 하고 메모리로부터 가져올 때도 캐싱 라인을 기준으로 가져온다. 종류로는 대표적으로 세 가지 방식이 존재한다.


1. Direct Map

메모리의 여러 개의 주소가 캐시 메모리의 한주소로 대응되는 다대일 방식이다. 하나의 메모리 주소가 주어졌을 때, 주소의 일부를 캐시 메모리의 인덱스로 사용한다. 따라서 캐시 메모리 내 메인 메모리의 주소가 있을 수 있는 장소가 고정된다. 간단하고 빠르지만, 같은 인덱스를 가지는 메인 메모리의 여러 개가 인접해서 참조될 때 캐시 미스(cache miss)가 지속적으로 발생한다.


2. Fully Associative

비어있는 캐시 메모리가 있으면 마음대로 주소를 저장하는 방식이다. 즉, 저장시 크게 알고리즘 비용이 없어 간단하고 캐시 미스 확률이 적지만, 블록을 찾을때 캐시 내의 모든 엔트리를 검색해야하는 단점이 있다.

3. Set Associative

Direct Map과 Fully Associative의 혼합한 방식이다. 캐시 내 하나의 엔트리에 들어갈 수 있는 블록 수가 정해져있다. 들어갈 수 있는 블록 수에 따라 , n-way set associative라고 한다. Direct Map에 비해 검색이 오래 걸리지만 저장이 빠르며, Fully Associative에 비해 저장이 느리지만 검색이 빠른 중간형이다.
