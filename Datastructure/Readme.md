...
해시 테이블의 버킷의 길이를 `n`, key의 수를 `m`이라고 가정했을 때, `α`는 1보다 작거나 같다. 저장소 1개 버킷 당 1개의 값만 가지기 때문이다.
`m/n = α ( α <= 1)`

삽입, 삭제, 탐색 모두 대상이 되는 해시를 찾아가는 과정에 따라 시간 복잡도가 계산된다. 해시 함수를 통해 얻은 해시가 비어있지 않으면 다음 버킷을 찾아가야 한다. 이 찾아가는 횟수가 많아지면 시간 복잡도가 증가한다. (`O(1) ~ O(n)`)
